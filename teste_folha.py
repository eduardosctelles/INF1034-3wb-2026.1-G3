import pygame
import sys

pygame.init()
screen = pygame.display.set_mode((800, 600))
pygame.display.set_caption("Folha Guerreira")
clock = pygame.time.Clock()
fonte = pygame.font.SysFont(None, 55)

class Player(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.image = pygame.Surface((40, 60))
        self.image.fill((0, 0, 255))
        self.rect = self.image.get_rect(center=(400, 500))
        
        # Física e Movimentação
        self.velocidade_x = 5
        self.velocidade_y = 0
        self.gravidade = 0.8
        self.forca_pulo = -15
        self.no_chao = False
        self.vivo = True

    def update(self, *args):
        if not self.vivo:
            return

        keys = pygame.key.get_pressed()
        
        # Andar para a esquerda e direita
        if keys[pygame.K_LEFT]:
            self.rect.x -= self.velocidade_x
        if keys[pygame.K_RIGHT]:
            self.rect.x += self.velocidade_x
            
        # Pular
        if keys[pygame.K_SPACE] and self.no_chao:
            self.velocidade_y = self.forca_pulo
            self.no_chao = False

        # Gravidade
        self.velocidade_y += self.gravidade
        self.rect.y += self.velocidade_y

        # Colisão com o chão
        if self.rect.bottom >= 550:
            self.rect.bottom = 550
            self.velocidade_y = 0
            self.no_chao = True

        # Limites da tela
        if self.rect.left < 0:
            self.rect.left = 0
        if self.rect.right > 800:
            self.rect.right = 800

class Leaf(pygame.sprite.Sprite):
    def __init__(self, x_inicial):
        super().__init__()
        try:
            self.sheet = pygame.image.load("Leaf-sheet.png").convert_alpha()
        except FileNotFoundError:
            # Fallback em cor verde sólida caso a imagem não exista
            self.sheet = pygame.Surface((256, 160))
            self.sheet.fill((0, 255, 0))

        # Configurações da Spritesheet 
        self.sprite_width = 32
        self.sprite_height = 32

        # Animação de andar
        self.walk_frames = []
        linha_andar = 0
        for coluna in range(8):  # 8 colunas para os frames 0 a 7
            frame = self.sheet.subsurface((coluna * self.sprite_width, linha_andar * self.sprite_height, self.sprite_width, self.sprite_height))
            self.walk_frames.append(frame)

        # Animação de atacar
        self.attack_frames = []
        linha_ataque = 2
        for coluna in range(7):  # 7 colunas para os frames 16 a 22
            frame = self.sheet.subsurface((coluna * self.sprite_width, linha_ataque * self.sprite_height, self.sprite_width, self.sprite_height))
            self.attack_frames.append(frame)

        # Animação de morrer
        self.death_frames = []
        linha_morte = 3
        for coluna in range(8):  
            frame = self.sheet.subsurface((coluna * self.sprite_width, linha_morte * self.sprite_height, self.sprite_width, self.sprite_height))
            self.death_frames.append(frame)

        # Movimentação e Estados
        self.inicio_x = x_inicial
        self.distancia_maxima = 100
        self.velocidade = 2
        self.direcao = 1 # 1 = direita, -1 = esquerda
        self.estado = "walk"
        self.frame_index = 0
        self.anim_timer = 0
        self.morta = False

        # Configura a dimensão visual.
        self.image = pygame.transform.scale(self.walk_frames[0], (128, 128))
        self.rect = self.image.get_rect(topleft=(x_inicial, 450))

    def iniciar_ataque(self):
        if not self.morta:
            self.estado = "attack"
            self.frame_index = 0  # Reinicia a animação para começar do frame 16

    def update(self, player):
        # Morte do player
        if self.estado == "dead":
            self.anim_timer += 1
            if self.anim_timer >= 5:
                self.anim_timer = 0
                if self.frame_index < len(self.death_frames) - 1:
                    self.frame_index += 1
                raw_img = self.death_frames[self.frame_index]
                self.image = pygame.transform.scale(raw_img, (128, 128))
            return

        # Lógica de movimentação lateral
        self.rect.x += self.velocidade * self.direcao
        if self.rect.x >= self.inicio_x + self.distancia_maxima:
            self.direcao = -1
        elif self.rect.x <= self.inicio_x - self.distancia_maxima:
            self.direcao = 1

        # Distância em relação ao jogador para mudar o estado de comportamento
        distancia = abs(player.rect.centerx - self.rect.centerx)
        if distancia < 150:
            if self.estado != "attack": # Evita sobrescrever a animação de ataque se já estiver atacando
                self.iniciar_ataque()
        elif not self.morta and self.estado != "attack":
            self.estado = "walk"

        # Controle de temporização da animação
        self.anim_timer += 1
        if self.anim_timer >= 5:
            self.anim_timer = 0
            
            if self.estado == "walk":
                self.frame_index = (self.frame_index + 1) % len(self.walk_frames)
                raw_img = self.walk_frames[self.frame_index]
            elif self.estado == "attack":
                # Finaliza o ataque se chegar ao final da lista, voltando a andar
                if self.frame_index >= len(self.attack_frames) - 1:
                    self.estado = "walk"
                    self.frame_index = 0
                else:
                    self.frame_index += 1
                raw_img = self.attack_frames[self.frame_index]
                
            self.image = pygame.transform.scale(raw_img, (128, 128))

# Configurações Iniciais
def resetar_jogo():
    global player, enemy, grupo_sprites
    player = Player()
    enemy = Leaf(500)
    grupo_sprites = pygame.sprite.Group()
    grupo_sprites.add(player)
    grupo_sprites.add(enemy)

resetar_jogo()

# Loop Principal
while True:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            pygame.quit()
            sys.exit()
            
        # Resetar o jogo caso aperte R
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_r and not player.vivo:
                resetar_jogo()

    if player.vivo:
        grupo_sprites.update(player)

    # Detecção de colisão entre o Personagem e o Inimigo
    if player.rect.colliderect(enemy.rect) and not enemy.morta:
        if player.velocidade_y > 0 and player.rect.bottom <= enemy.rect.top + 30:
            enemy.morta = True
            enemy.estado = "dead"
            enemy.frame_index = 0
            player.velocidade_y = -12
        else:
            enemy.iniciar_ataque()
            player.vivo = False

    # Renderização da Janela
    screen.fill((0, 0, 0))
    pygame.draw.rect(screen, (255, 255, 255), (0, 550, 800, 50)) 
    grupo_sprites.draw(screen)
    
    # Game Over
    if not player.vivo:
        texto_morte = fonte.render("VOCE MORREU - APERTE R", True, (255, 0, 0))
        texto_rect = texto_morte.get_rect(center=(400, 200))
        screen.blit(texto_morte, texto_rect)

    pygame.display.flip()
    clock.tick(60)